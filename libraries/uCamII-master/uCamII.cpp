...
#endif
        for (int i = 0; i < 6; i++) {
          Serial3.write(_SYNC_FINAL_COMMAND[i]);
        }
        return 1;
      }
    }
...
#endif
  for (int i = 0; i < 6; i++) {
    Serial3.write(_INITIAL_COMMAND[i]);
  }
  // @todo why 500 delay?
  delay(500);
...
int UCAMII::getData() {
  ...
  for (i = 0; i < bytes; i++) {
    while (!Serial3.available()); // wait for bytes
    s = Serial3.read();
